...
static uint16_t AudioInVolume = DEFAULT_AUDIO_IN_VOLUME;static void DFSDMx_FilterMspInit(DFSDM_Filter_HandleTypeDef *hdfsdm_filter);
...
static uint8_t DFSDMx_Init(uint32_t AudioFreq, uint32_t ChnlNbr)
{
  ...
  if (ChnlNbr == 1)
  {
    ...
    if (ChnlNbr >= 2)
    {
      ...
      haudio_in_dfsdmchannel[1].Instance                      = DFSDM_Channel5;      if(HAL_OK != HAL_DFSDM_ChannelInit(&haudio_in_dfsdmchannel[1]))
      {
        return AUDIO_ERROR;
      }
      /*####AUDIO CH 2: FILTER 1####*/
