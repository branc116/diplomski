...
static SPI_HandleTypeDef SPI_Sensor_Handle;
 ...
 DMA_HandleTypeDef hdma_tx;GPIO_TypeDef* GPIO_PORT[LEDn] = {LED1_GPIO_PORT, LEDSWD_GPIO_PORT};
...
{
  DrvContextTypeDef *ctx = (DrvContextTypeDef *)handle;  return I2C_SENSORTILE_WriteData( ctx->address, WriteAddr, pBuffer, nBytesToWrite );
}/**
...
uint8_t Sensor_IO_I2C_Read( void *handle, uint8_t ReadAddr, uint8_t *pBuffer, uint16_t nBytesToRead )
{
  ...
  DrvContextTypeDef *ctx = (DrvContextTypeDef *)handle;  return I2C_SENSORTILE_ReadData( ctx->address, ReadAddr, pBuffer, nBytesToRead );
}/**
 * @brief  Writes a buffer to the sensor
 * @param  handle instance handle
 * @param  WriteAddr specifies the internal sensor address register to be written to
...
static void I2C_SENSORTILE_MspInit( void )
{
  ...
  SENSORTILE_I2C_ONBOARD_SENSORS_SCL_SDA_GPIO_CLK_ENABLE();  GPIO_InitStruct.Pin        = SENSORTILE_I2C_ONBOARD_SENSORS_SCL_PIN | SENSORTILE_I2C_ONBOARD_SENSORS_SDA_PIN;
...
static void SD_IO_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
  ...
  /* Enable DMA clock */
  __HAL_RCC_DMA2_CLK_ENABLE();  /*** Configure the GPIOs ***/
  /* Enable GPIO clock */
