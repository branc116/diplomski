...
{
  uint8_t tmp;  LPS22HB_assert_param(IS_LPS22HB_ODR(odr));
...
LPS22HB_Error_et LPS22HB_Set_LowPassFilter(void *handle, LPS22HB_State_et state)
{
  ...
  tmp |= ((uint8_t)state) << LPS22HB_LPFP_BIT;  if(LPS22HB_WriteReg(handle, LPS22HB_CTRL_REG1, 1, &tmp))
    return LPS22HB_ERROR;
  return LPS22HB_OK;
}/**
...
LPS22HB_Error_et LPS22HB_Set_LowPassFilterCutoff(void *handle, LPS22HB_LPF_Cutoff_et cutoff)
{
  ...
  tmp |= (uint8_t)cutoff;  if(LPS22HB_WriteReg(handle, LPS22HB_CTRL_REG1, 1, &tmp))
    return LPS22HB_ERROR;
...
{
  uint8_t tmp;  LPS22HB_assert_param(IS_LPS22HB_BDUMode(bdu));

  if(LPS22HB_ReadReg(handle, LPS22HB_CTRL_REG1, 1, &tmp))
    return LPS22HB_ERROR;

  tmp &= ~LPS22HB_BDU_MASK;
  tmp |= ((uint8_t)bdu);  if(LPS22HB_WriteReg(handle, LPS22HB_CTRL_REG1, 1, &tmp))
    return LPS22HB_OK;
...
{
  uint8_t tmp;  LPS22HB_assert_param(IS_LPS22HB_SPIMode(spimode));
...
{
  uint8_t tmp;  LPS22HB_assert_param(IS_LPS22HB_CTE(mode));

  if(LPS22HB_ReadReg(handle, LPS22HB_CLOCK_TREE_CONFIGURATION, 1, &tmp))
    return LPS22HB_ERROR;

  tmp &= ~LPS22HB_CTE_MASK;
  tmp |= (uint8_t)mode;  if(LPS22HB_WriteReg(handle, LPS22HB_CLOCK_TREE_CONFIGURATION, 1, &tmp))
    return LPS22HB_ERROR;
...
{
  uint8_t tmp;  LPS22HB_assert_param(IS_LPS22HB_State(status));
...
LPS22HB_Error_et LPS22HB_Get_ReferencePressure(void *handle, int32_t* RefP)
{
  ...
  *RefP = (raw_press * 100) / 4096;  return LPS22HB_OK;
}/**
...
LPS22HB_Error_et LPS22HB_Get_FifoConfig(void *handle, LPS22HB_FIFOTypeDef_st* pLPS22HBFIFO)
{
  ...
  pLPS22HBFIFO->WTM_INT = (LPS22HB_State_et)((tmp & LPS22HB_WTM_EN_MASK) >> LPS22HB_WTM_EN_BIT);  return LPS22HB_OK;
}
