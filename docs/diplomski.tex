...
\subsubsection{GATT}
Bluetooth \textbf{G}eneric \textbf{ATT}ribue Profile definira koji podaci postoje na pojedinim bluetooth uređajima. Definira na način da svaki uređaj ima svoje servise koje su identificirani sa jedinstvenim brojem (engl. UUID). UUID može biti veličine 16 bajta ili 2 bajta. Neki brojevi su rezervirani u bluetooth standardu\cite{core41}.
...
GCC Arm\cite{armgcc} je alat koji pomaže programeru pretvoriti izvorni kod pisan u C programskom jeziku u binarni format kojeg je u konačnici moguce pokrenuti na mikrokontrolerima sa ARM procesorom. Ustvari ovo je samo druga verzija(engl. fork) GCC\cite{gcc} programa, verzija koja je specijalizirana za ARM procesore.
